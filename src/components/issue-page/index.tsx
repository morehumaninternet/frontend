...
function useDemo(
  issue: Issue,
  postComment: LoadedIssueContentProps['postComment'],
  changeStatus: LoadedIssueContentProps['changeStatus']
) {
  if (issue.status === 'Opened') {
    setTimeout(() => {
      changeStatus(
        { username: 'devdiva', avatarUrl: '/devdiva.png' },
        'Acknowledged',
        {
...
export default function LoadedIssueContent({
  avatarUrl,
  issue,
  postComment,
  changeStatus,
}: LoadedIssueContentProps): JSX.Element {
