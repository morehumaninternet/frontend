import { ScreenshotWidgetStore } from '../stores/screenshot-widget'
import * as mockApi from '../clients/mockApi'

export default function subscribe(store: ScreenshotWidgetStore, api: typeof mockApi): void {
  let previousState: ScreenshotWidgetState = store.getState() // tslint:disable-line:no-let

  store.subscribe(() => {
    const nextState: ScreenshotWidgetState = store.getState()
